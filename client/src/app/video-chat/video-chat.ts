...
@Component({
  ...
  template: `
    <div class="relative h-full w-full">
      <video class="w-32 h-52 absolute right-5 top-4 object-cover border-red-500 border-2 rounded-lg"
...
export class VideoChat {
  ...
  setupSignalListeners() {
    this.signalRService.hubConnection.on('CallEnded', () => {
      this.closeLocalCall();
      this.snackBar.open('Call was ended', 'Close', { duration: 5000 });
    });
  ...
  async acceptCall() {
    ...
    if (offer) {
      ...
      await this,this.peerConnection.setLocalDescription(answer);
